    ...
    <div class="certificate-frame" style="background-image: url('{{public_path($certificate->image)}}'); background-size: 100% auto; background-repeat: no-repeat; background-position: center;">
        <div style="position: relative; font-family: 'Mongolian Baiti';  padding: 35px  0;">
            ...
            @if($certificate->show_date == 'yes')
                <div class="certificate-publish-date" style="padding-top: 40px; text-align: center; position: relative; top: {{$certificate->date_y_position? : 10}}px; font-size: {{$certificate->date_font_size? : 30}}px; font-weight: 400; color: {{$certificate->date_font_color? : '#363234'}}; padding-bottom: 16px">{{\Carbon\Carbon::now()->format(get_option('app_date_format'))}}</div>
            @endif
            @if($certificate->show_student_name == 'yes')
                <div class="certificate-student-name" style="text-align: center; position: relative; top: {{$certificate->student_name_y_position? : 10}}px; font-size: {{$certificate->student_name_font_size? : 32}}px; font-weight: 400;  color: {{$certificate->student_name_font_color? : '#363234'}}; padding-bottom: 16px">{{auth::user()->student ? auth::user()->student->name : '' }}</div>
            @endif
            <p class="certificate-content" style="padding-bottom: 24px; position: relative; top: {{$certificate_by_instructor->body_y_position? : 16}}px; text-align: center; padding-left: 128px; padding-right: 128px; color: {{$certificate_by_instructor->body_font_color? : '#363234'}}; font-size: {{$certificate_by_instructor->body_font_size? : 20}}px;">
