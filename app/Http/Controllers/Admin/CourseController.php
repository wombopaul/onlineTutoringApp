...
class CourseController extends Controller
{
    ...
    public function statusChange($uuid, $status)
    {
        ...
        if ($status == 1) {
            ...
            $taget_url = route('course-details', $course->slug);
            $this->send($text, 2, $taget_url, $course->user_id);

            /** ====== send notification to student ===== */
            $students = Student::where('user_id', '!=', $course->user_id)->select('user_id')->get();
            foreach ($students as $student) {
                $text = "New course has been published";
                $taget_url = route('course-details', $course->slug);
        ...
        if ($status == 3) {
            $text = "Course has been hold";
            $taget_url = route('instructor.course');
            $this->send($text, 2, $taget_url, $course->user_id);
        }
    ...
    public function courseEnrollStore(Request $request)
    {
            ...
            if ($courseOrderExits) {
                ...
                if ($order) {
                    if ($order->payment_status == 'due') {
                        ...
                        $this->showToastrMessage('error', "Student has already purchased the course!");
                        return redirect()->back();
                    }
                }
            }
        ...
        if ($ownCourseCheck) {
            $this->showToastrMessage('error', "He is a owner of the course. Can't purchase this course!");
            return redirect()->back();
        }
        ...
        $text = "New course enrolled by Admin";
        $target_url = route('student.my-learning');
        $this->send($text, 3, $target_url, $request->user_id);

        /** ====== Send notification =========*/

        $this->showToastrMessage('success', 'Student enroll in course');
        return redirect()->back();
    }
}
